...
\begin{frame}{TbCo$_2$: A Ferrimagnetic Compound}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \begin{itemize}
...
\begin{frame}{Transport Properties of TbCo$_2$}
  \begin{columns}[T]
    \column{0.45\textwidth}
    \begin{itemize}
      \item Non-linear Hall resistivity $\rho_{xy}$ — characteristic of \textbf{Anomalous Hall Effect} (AHE)
      \item \textbf{Sign Flip of Anomalous Hall and carrier} observed near $T_C = 232\,$K
      \item We want to explain the high Tc of magnetism and the origin of these transport signatures    \end{itemize}
...
\begin{frame}{A view of thermalization from two different initial states}
  \begin{columns}[T]
    \column{0.45\textwidth}
    \begin{itemize}
      \item Choose two different initial configurations (e.g.\ all spins up vs.\ random)
    \end{itemize}
    \vspace{0.5em}
    ...
    \begin{tikzpicture}[scale=0.85]
      ...
      \node[font=\scriptsize, accentblue] at (1.8,-0.3) {seed = 42 (random)};
...
% ------------------------------------------------------------------
\begin{frame}{Dynamic Critical Exponent: Numerical Measurement}
  \begin{block}{Definition}
    The \textbf{integrated autocorrelation time} $\tau_{\text{int}}$ of an observable
    (here $\lvert m\rvert$) scales with system size near $T_c$ as
    $\tau_{\text{int}} \propto L^z$, where $z$ is the \textbf{dynamic critical exponent}.
    Measured from time series at $T_c$ for $L = 8, 16, 32, 64$.
  \end{block}

  \vspace{-0.2em}
  \begin{center}
    \includegraphics[width=0.78\textwidth,height=0.54\textheight,keepaspectratio]{dynamic_exponent_z.png}
  \end{center}

  \vspace{-0.5em}
  {\footnotesize Cluster algorithms ($z \sim 0.2$--$0.4$) eliminate most of the
   critical slowing down that plagues Metropolis ($z \sim 2$).}
\end{frame}
